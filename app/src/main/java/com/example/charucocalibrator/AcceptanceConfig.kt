package com.example.charucocalibrator

object AcceptanceConfig {
    const val MIN_CHARUCO_CORNERS = 40
    const val SHARPNESS_THRESHOLD = 100.0
    const val MIN_BBOX_AREA_RATIO = 0.15
    const val MIN_TIME_BETWEEN_ACCEPTED_MS = 900L
    const val MAX_ACCEPTED_FRAMES = 30
    /** Normalized scales for pose-distance (see [PoseSimilarity]). */
    const val POSE_CENTER_SCALE = 0.10
    const val POSE_AREA_SCALE = 0.03
    const val POSE_ASPECT_SCALE = 0.06
    /** Relative corner-count difference treated as one unit in pose distance. */
    const val POSE_CORNER_SCALE = 0.20
    /** Reject when pose distance to any prior accept is below this (0 = identical). */
    const val MAX_POSE_SIMILARITY_DISTANCE = 1.0
    const val MIN_FRAMES_FOR_CALIBRATION = 10
    const val MAX_PER_VIEW_REPROJECTION_ERROR_PX = 3.0
    const val MAX_ISO = 1000
    /** ~1/30 s; indoor AE often lands around 15–25 ms on phone cameras. */
    const val MAX_EXPOSURE_TIME_NS = 33_000_000L
    /** Max relative focus-distance oscillation (diopters) in a short window; catches AF hunting. */
    const val MAX_FOCUS_RELATIVE_STD_DEV = 0.08f
    const val MIN_FOCUS_SAMPLES_FOR_VARIANCE = 5
    const val REQUIRE_CAPTURE_METADATA = true
    const val MIN_VIEWS_FOR_RATIONAL_MODEL = 25
}
